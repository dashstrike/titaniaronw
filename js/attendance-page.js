...
(function(){
  ...
  function normalizeActual(v){return ACTUAL_META[v]?v:null;}
  ...
  function counts(rows){const c={going:0,not_going:0,no_response:0,present:0,absent:0,excused:0,afk:0,unmarked:0};rows.forEach(r=>{c[r.pre_status]++;if(r.actual_status)c[r.actual_status]++;else c.unmarked++;});return c;}
  function filteredRows(rows){const q=filters.search.trim().toLowerCase();return rows.filter(r=>(filters.pre==='all'||r.pre_status===filters.pre)&&(filters.actual==='all'||(filters.actual==='unmarked'?!r.actual_status:r.actual_status===filters.actual))&&(!q||`${r.name} ${r.cls} ${r.assignment&&r.assignment.partyLabel||''} ${r.assignment&&r.assignment.raidLabel||''}`.toLowerCase().includes(q)));}
  ...
  function renderEventPanel(){
    ...
    panel.innerHTML=`<div class="att-event-head"><div><span class="att-event-kicker">${EVENT_META[selectedEvent.event_type].icon} ${esc(eventTitle(selectedEvent))}</span><h2>${esc(formatEventDate(selectedEvent.event_date))}</h2><p>${closed?'Attendance finalized':'Mark actual attendance. Changes save immediately.'}</p></div><div class="att-event-actions">${refreshMembersButton}${finishButton}</div></div><div class="att-summary-grid">${chip(c.going,'Pre Going','going')}${chip(c.not_going,'Pre Not Going','not-going')}${chip(c.no_response,'No Response','no-response')}${chip(c.present,'Present','present')}${chip(c.absent,'Absent','absent')}${chip(c.excused,'Excused','excused')}${chip(c.afk,'AFK','afk')}${chip(c.unmarked,'No Status','unmarked')}${chip(`${rate}%`,'Attendance Rate','rate')}</div><div class="att-filterbar"><input id="attSearch" type="search" placeholder="Search member, class, party…" value="${esc(filters.search)}"><select id="attPreFilter"><option value="all">All Pre</option><option value="going">Going</option><option value="not_going">Not Going</option><option value="no_response">No Response</option></select><select id="attActualFilter"><option value="all">All Actual</option><option value="unmarked">No Status</option><option value="present">Present</option><option value="absent">Absent</option><option value="excused">Excused</option><option value="afk">AFK</option></select></div><div class="att-column-head"><span>Member</span><span>Party</span><span>Pre</span><span>Actual</span><span></span></div><div id="attendanceMemberGroups"></div>`;
  ...
  async function saveActual(id,status){if(!selectedEvent||!canEdit()||selectedEvent.status==='closed')return;const member=membersForEvent().find(r=>r.memberId===id);if(!member)return;const nextStatus=member.actual_status===status?null:status;const payload={event_id:selectedEvent.id,member_id:id,member_name:member.name,pre_status:member.pre_status,actual_status:nextStatus,note:member.note||''};const r=await supabaseClient.from('attendance_records').upsert(payload,{onConflict:'event_id,member_id'}).select('*').single();if(r.error){toast('Attendance save failed',r.error.message,'err');return;}records.set(id,{...r.data,pre_status:normalizePre(r.data.pre_status),actual_status:normalizeActual(r.data.actual_status)});renderEventPanel();}
  ...
  async function finishAttendance(){if(!selectedEvent||!canEdit())return;if(!window.confirm('Finish and close this attendance record? Members with no status will stay unmarked.'))return;const close=await supabaseClient.from('attendance_events').update({status:'closed',closed_at:new Date().toISOString()}).eq('id',selectedEvent.id);if(close.error){toast('Finish attendance failed',close.error.message,'err');return;}selectedEvent.status='closed';await fetchRecords(selectedEvent.id);await fetchEvents();renderShell();}
