(function TitaniaPartyStars(){
  ...
  function isPublicGuild(){ return /\/guild-league\.html$/i.test(location.pathname); }
  function isPublicSiege(){ return /\/siege\.html$/i.test(location.pathname); }

  function resolveEvent(){
    if(isManagement()){
      try{
        if(typeof currentEvent !== 'undefined' && currentEvent === 'guild_league') return 'guild_league';
        if(typeof currentEvent !== 'undefined' && currentEvent === 'siege') return 'siege';
      }catch(_e){}
      return '';
    }
    if(isPublicGuild()) return 'guild_league';
    if(isPublicSiege()) return 'siege';
    return '';
  }
  ...
  function getClient(){
    ...
    try{ if(typeof window.client !== 'undefined' && window.client) return client=window.client; }catch(_e){}
    try{ if(typeof client !== 'undefined' && client) return client=client; }catch(_e){}
  ...
  function managementCards(){
    return [...document.querySelectorAll('.team-card[data-team-key]')];
  }

  function publicTeamKeys(){
    if(isPublicGuild()){
      return [
        ...Array.from({length:12},(_,i)=>`main_${i+1}`),
        'sub_1','sub_2','sub_3','sub_4','sub_17',
        'sub_5','sub_6','sub_7','sub_8','sub_18',
    ...
    if(isPublicSiege()){
      try{
        if(typeof currentSiegeRaids==='function') return currentSiegeRaids().flatMap(raid=>raid.keys||[]);
      }catch(_e){}
      return Array.from({length:20},(_,i)=>`siege_main_${i+1}`);
    }
    return [];
  }
  ...
  function publicCards(){
    ...
    const keys=publicTeamKeys();
    return cards.map((card,index)=>({card,key:keys[index]||''})).filter(x=>x.key);
  }
  ...
  function updateCounter(){
    ...
    counter.title=`Maximum ${MAX_STARS} parties can be marked for Star`;
  }

  function decorateManagement(){
    const active=resolveEvent();
    if(active!==eventKey)return;
  ...
  function decorate(){
    queued=false;
    if(!eventKey)return;
    if(isManagement())decorateManagement(); else decoratePublic();
  }
  ...
  async function loadSelections(force=false){
    ...
    if(!next){ eventKey=''; selected.clear(); schedule(); return; }
    if(loading)return;
    if(!force && next===eventKey && selected.size>=0){ schedule(); return; }
    const c=getClient();
    if(!c)return;
    loading=true;
    try{
      const {data,error}=await c.from(TABLE).select('team_key').eq('event_key',next);
      if(error)throw error;
      eventKey=next;
      selected=new Set((data||[]).map(row=>String(row.team_key||'')).filter(Boolean));
      schedule();
    }catch(error){
      console.warn('[Titania] Could not load Star parties:',error);
    }finally{ loading=false; }
  }

  async function toggle(key){
    if(!canEdit()||!key||!eventKey)return;
    const c=getClient(); if(!c)return;
    const on=selected.has(key);
    if(!on && selected.size>=MAX_STARS){
      try{ if(typeof showToast==='function')showToast('Star Party limit',`You can only mark ${MAX_STARS} parties for Star.`,'warn'); else alert(`Maximum ${MAX_STARS} Star parties.`); }catch(_e){}
      return;
    }
    ...
    try{
      let result;
      if(on) result=await c.from(TABLE).delete().eq('event_key',eventKey).eq('team_key',key);
      else result=await c.from(TABLE).insert({event_key:eventKey,team_key:key});
      if(result.error)throw result.error;
      try{ if(typeof touchEventUpdate==='function')touchEventUpdate(eventKey); }catch(_e){}
      try{ if(typeof queueSave==='function')queueSave(); }catch(_e){}
      try{ if(typeof showToast==='function')showToast(on?'Star removed':'Star Party selected',on?'Party is no longer marked for Star.':`${selected.size}/${MAX_STARS} parties selected for Star.`); }catch(_e){}
    }catch(error){
  ...
  function bind(){
    ...
    const observer=new MutationObserver(()=>{
      const next=resolveEvent();
      if(next!==eventKey)loadSelections(true); else schedule();
    });
    observer.observe(document.body,{subtree:true,childList:true});

    window.addEventListener('focus',()=>loadSelections(true));
    document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadSelections(true);});
  }

  function boot(){
    if(!(isManagement()||isPublicGuild()||isPublicSiege()))return;
    style();bind();loadSelections(true);
  }
